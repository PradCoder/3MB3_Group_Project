...
ss <- function(par = c(a,c,e)) {
  with(as.list(par), {
    ...
    #h=1 for one year increases, dp = a*(1-(canada[i]/40000))*usa[i] - c*canada[i] + e
...
for (i in 1:(length(C_vals) - 1)) {
  C_vals[i + 1] <- canada[i]+ a_fit*(1-(canada[i]/40000))*usa[i] - c_fit*canada[i] + e_fit
}
...
ss <- function(par = c(b,d,f)) {
  with(as.list(par), {
    ...
    #using Euler's method to predict spending in t+1
    #h=1 for one year increases, dg = b*(1-(usa[i]/1041000))*canada[i]-d*usa[i]+f
...
#run the model with calibrated parameters 
for (i in 1:(length(U_vals) - 1)) {
  U_vals[i + 1] <- usa[i]+ b_fit*(1-(usa[i]/1041000))*canada[i]-d_fit*usa[i]+f_fit
}
